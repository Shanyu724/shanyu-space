    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10 md:py-14">
      ...
      <FadeIn>
        <section className="relative min-h-[360px] flex flex-col items-center justify-center text-center overflow-visible card-polaroid mb-20 px-6 py-12">
          ...
          {/* 共创小标签（黄底贴纸感） */}
          <span className="relative z-10 inline-block text-xs tracking-[0.35em] text-rose-500 mb-3 px-2 py-0.5 rounded-sm -rotate-2" style={{ backgroundColor: "rgba(235,197,133,0.4)" }}>
            共创
          </span>
          ...
          <p className="relative z-10 mt-6 text-sm md:text-base text-mint-700 leading-relaxed max-w-xl">
            <span className="text-rose-400">✎</span> Claude 写代码，Gemini 画草图，
            我负责判断什么值得留下，然后上线 ☁
          </p>
    ...
    >
      ...
      <div className="relative h-48 overflow-hidden bg-mint-50">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <span className="text-7xl drop-shadow-sm select-none">{project.emoji || "✦"}</span>
        </div>
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="flex flex-wrap gap-1.5">
            {project.tags.slice(0, 3).map((tag) => (
              <span key={tag} className="text-xs px-2 py-0.5 rounded-full bg-mint-50 border border-mint-100 text-mint-600">
                {tag}
              </span>
            ))}
